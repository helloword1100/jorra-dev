...
export class HairstyleService {
  static async getHairstyles(params?: {
    ...
    try {
      console.log("[v0] Fetching hairstyles from:", `${API_BASE_URL}/hairstyles?${searchParams}`)
      const response = await fetch(`${API_BASE_URL}/hairstyles?${searchParams}`, {
        headers: {
  ...
  static async getCategories(): Promise<{ categories: Array<{ id: number; name: string; description?: string }> }> {
    try {
...
export class TryOnService {
  static async applyHairstyleById(hairstyleId: number, selfieFile: File): Promise<Blob> {
    const formData = new FormData()
  ...
  static async applyHairstyleWithUpload(selfieFile: File, hairstyleFile: File): Promise<Blob> {
    const formData = new FormData()
    formData.append("selfie", selfieFile)
  ...
  static async getMyGenerations(params?: {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/my-generations?${searchParams}`, {
        headers: AuthService.getAuthHeaders(),
      })
  ...
  static async generateVideo(generationId: number): Promise<Blob> {
    try {
      const response = await fetch(`${API_BASE_URL}/generate-video/${generationId}`, {
        method: "POST",
        headers: AuthService.getAuthHeaders(),
      })
  ...
  static async claimSocialMediaBonus(postUrl: string): Promise<{ success: boolean; credits_added: number }> {
    try {
      const formData = new FormData()
  ...
  static async shareGeneration(
    ...
    const response = await fetch(`${API_BASE_URL}/share-generation/${generationId}`, {
      method: "POST",
      headers: AuthService.getAuthHeaders(),
      body: formData,
    })
...
export class AdminService {
  static async getTryOnRequests(params?: {
    ...
    const response = await fetch(`${API_BASE_URL}/admin/tryon-requests?${searchParams}`, {
      headers: AuthService.getAuthHeaders(),
    })
  ...
  static async approveTryOnRequest(requestId: number): Promise<{
    ...
    const response = await fetch(`${API_BASE_URL}/admin/approve-tryon-request/${requestId}`, {
      method: "PUT",
      headers: AuthService.getAuthHeaders(),
    })
  ...
  static async denyTryOnRequest(requestId: number): Promise<{
    ...
    const response = await fetch(`${API_BASE_URL}/admin/deny-tryon-request/${requestId}`, {
      method: "PUT",
      headers: AuthService.getAuthHeaders(),
    })
